...
import { updateDocument, showFailureModal, removeDocument } from '../actions';
...
function *getPDFFromStore() {
    ...
    function *task(action: Sign.Actions.AddPDFToStoreAction) {
        ...
        try{
            ...
            yield put(showFailureModal({message: 'Sorry, we cannot read this PDF file', title: 'Read PDF Failed'}))
            try{
                yield put(removeDocument(action.payload.id))
            }catch(e){
